...
Submitted datasets are reviewed by the \mqcas maintainers for integrity and often require comparisons to literature values to ensure consistency before accepting a contribution.
...
In addition to forming a complete platform, the four software components were carefully constructed to be usable outside of the \qca stack to maximize community utility and reusability.
A brief overview of each component can be found below:

\begin{itemize}
    \item \qcsk - A community built key-value/array description of QC (or QC-like) objects such as molecules and input/output quantities from QC programs. \url{https://github.com/MolSSI/QCSchema}
...
Fundamentally \qcf is a campaign manager designed for long-duration (years) deployments to passively manage all computation and data storage requirements for an individual researcher or groups of researchers concurrently.
...
For example, unique identifiers for given single point computation are molecule, method, basis, program, and keyword arguments create a unique computation.
...
A \qcf quick start guide to reproduce the above example can be found on our website \cite{QCFractal:Quickstart}.
